...
class AnchorTarget:
    def __init__(self,):
        ...
        self.anchors.generate_all_anchors(im_c=cfg.TRAIN.SEARCH_SIZE//2, #255//2
                                          size=cfg.TRAIN.OUTPUT_SIZE)  #29

    def __call__(self, target, size, neg=False):
        anchor_num = len(cfg.ANCHOR.RATIOS) * len(cfg.ANCHOR.SCALES) ## 1.计算每个锚点处anchor_num个锚框的宽高尺寸
       ...
       # cls_weight=np.ones((anchor_num, size, size), dtype=np.float32)
        ...
        #进行坐标转换
        tcx, tcy, tw, th = corner2center(target) #ground truth bbox
        ...
        #Positive anchor threshold 0.6 # Negative anchor threshold 0.3
        pos = np.where(overlap > cfg.TRAIN.THR_HIGH)
        neg = np.where(overlap < cfg.TRAIN.THR_LOW)
        # Number of positive=16
